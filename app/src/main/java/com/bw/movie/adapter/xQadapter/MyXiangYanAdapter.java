 ...
 * 功能：
 **/
public class MyXiangYanAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {

    Context context;
    List<XiangBean.ResultBean.MovieActorBean> movieActor;
    public MyXiangYanAdapter(Context context, List<XiangBean.ResultBean.MovieActorBean> movieActor) {
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof ViewHolder1) {
            ((ViewHolder1) holder).yanyuan_name.setText(movieActor.get(position).getName());
            Uri parse = Uri.parse(movieActor.get(position).getPhoto());
            ((ViewHolder1) holder).yanyuan_image.setImageURI(parse);
        }
    }
